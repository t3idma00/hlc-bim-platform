  ...
  return [
    {
      ...
      rows: [
        ...
        { id: "1.5", values: { item: "Glass", direction: "Single glass", type: "Glass only (Centre of Glass)", thickness: "6", uFactor: "", cltd: "", calcValue: "", heatLoad: "" }, selectOptions: { direction: heatLoadLookupOptions.transmissionGlassTypes, type: heatLoadLookupOptions.glassFrameTypes, thickness: heatLoadLookupOptions.glassThicknesses } },
        { id: "1.6", values: { item: "Roof", direction: "Intermediate floor", type: "Concrete", thickness: "", uFactor: "", cltd: "", calcValue: "", heatLoad: "" } },
      ],
    },
    // You can add sections 2–6 here later if needed
  ];
}
...
export function HeatLoadSheet({ sheetValues, onSheetChange }: Props) {
  ...
  // ==================== LOAD SAVED DATA ====================
  useEffect(() => {
    if (Object.keys(sheetValues).length === 0) return;
        ...
        rows: section.rows.map((row) => {
          ...
          Object.entries(sheetValues).forEach(([key, value]) => {
            if (key.startsWith(`${row.id}_`)) {
              const cellKey = key.split("_")[1]; // "uFactor", "cltd", "calcValue", "heatLoad", etc.
              if (cellKey && newValues.hasOwnProperty(cellKey)) {
  ...
  function handleCellChange(sectionNumber: string, rowId: string, key: string, value: string) {
    ...
    const fieldKey = `${rowId}_${key}`;
    onSheetChange(fieldKey, value);
  }
...
// ====================== HELPER COMPONENTS ======================
// (Keep all your existing SectionTable, SummaryTable, SheetCell, SheetInputCell, SheetSelectCell functions below)
            ...
            <tr key={row.label}>
              ...
              <td className={`${cellClass} bg-white p-0`}>
                <input
                  ...
                  className="min-h-[24px] w-full bg-transparent px-2 py-1 text-[10px] outline-none text-slate-900 text-right"
                />
              </td>
              <td className={`${cellClass} bg-white p-0`}>
                <input
...
// Cell components (SheetCell, SheetInputCell, SheetSelectCell) - keep them as they are
function SheetCell({ ariaLabel, value = "", align = "left", wrap = true }: { ariaLabel: string; value?: string; align?: Align; wrap?: boolean; }) {
